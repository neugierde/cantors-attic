...
(Redirected from
[N-remarkable](index.php?title=N-remarkable&redirect=no "N-remarkable"))
...
natural number $n$. \[[3](#bibkey_Kanamori2009:HigherInfinite)\]
...
-   For $A$-extendible $κ$ is, a set $g ⊆ κ$ is called
    **$A$-stretchable**, if for every $λ &gt; κ$ and every $h ⊆ λ$ for
    which $h ∩ κ = g$, there is an elementary embedding $j : ⟨V\_λ , ∈,
...
-   For $A$-extendible $κ$, a function $\\ell : κ → V\_κ$ is called an
    **$A$-extendibility Laver function**, if for every $λ$ and every
    target $a$, there is an elementary embedding $j : ⟨V\_λ , ∈, A ∩
    V\_λ⟩ → ⟨V\_θ , ∈, A ∩ V\_θ⟩$ such that $crit(j)=κ$, $λ &lt; j(κ)$
    and $j(\\ell)(κ) = a$.
...
-   In $\\text{GBC}$, for any class $A$ there is a class function $\\ell
    : \\mathrm{Ord} → V$, such that for every $A$-extendible cardinal
    $κ$, $\\ell ↾ κ$ is an $A$-extendible Laver function for $κ$.
